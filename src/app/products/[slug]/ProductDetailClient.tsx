  ...
  return (
    <>
      ...
      <main className="pt-[72px]">
        {/* Breadcrumb */}
        <div className="px-12 py-4 border-b border-white/7 flex items-center gap-2 text-[11px] tracking-[0.1em] text-muted/60">
          ...
          <span>/</span>
          <span className="text-[#f0ede6]">{product.name}</span>
        </div>

        {/* Main grid */}
        <div className="grid grid-cols-2 gap-px bg-white/7 min-h-[80vh]">
          {/* Gallery */}
          <div className="bg-[#131210] sticky top-[72px] self-start">
            <div className="aspect-square flex items-center justify-center">
              <span className="text-[160px] select-none opacity-60">{product.emoji}</span>
            </div>
            {/* Thumbnails strip */}
            <div className="flex gap-px border-t border-white/7">
              {[...Array(4)].map((_, i) => (
                <div key={i}
                  className={`flex-1 aspect-square bg-[#1a1916] flex items-center justify-center text-5xl cursor-pointer transition-opacity ${i === 0 ? 'opacity-100 ring-1 ring-inset ring-[#c8a96e]' : 'opacity-40 hover:opacity-70'}`}>
                  {product.emoji}
                </div>
              ))}
            </div>
          ...
          {/* Info */}
          <div className="bg-[#0b0a08] px-16 py-16">
            <p className="text-[10px] tracking-[0.3em] uppercase text-[#c8a96e] mb-3">{product.tag}</p>
            <h1 className="font-serif text-[42px] font-light leading-[1.15] mb-2">{product.name}</h1>
            ...
            {/* Price */}
            <div className="flex items-baseline gap-4 mb-8 pb-8 border-b border-white/7">
              <span className="font-serif text-5xl font-light text-[#c8a96e]">{formatPrice(product.price)}</span>
              {product.originalPrice && (
                <>
                  <span className="text-xl text-muted/60 line-through">{formatPrice(product.originalPrice)}</span>
            ...
            {/* Description */}
            {product.description && (
              <p className="text-[15px] text-muted leading-[1.9] mb-8">{product.description}</p>
            )}
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[12px] tracking-[0.1em] uppercase text-muted">Chọn size</p>
                <button className="text-[12px] text-[#c8a96e] hover:underline">Bảng size →</button>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 pt-8 border-t border-white/7">
              {[
                ...
                <div key={b.title} className="text-center">
                  ...
                  <p className="text-[11px] text-muted/60 mt-0.5">{b.sub}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {related.length > 0 && (
          <section className="px-12 py-20">
            <div className="mb-10">
              ...
              <h2 className="font-serif text-[36px] font-light">Sản phẩm tương tự</h2>
            </div>
